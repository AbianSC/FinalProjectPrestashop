  ...
  <div class="thumbnail-container">
    ...
    <div class="product-description">
      {block name='product_name'}
        <h4 class="h3 product-title" itemprop="name"><a href="{$product.url|escape:'html':'UTF-8'}">{$product.name|truncate:30:'...'|escape:'html':'UTF-8'}</a></h4>
      {/block}
             ...
             <div class="add_to_cart_button atc_div">
                  {*<form action="{if isset($urls)}{$urls.pages.cart|escape:'html':'UTF-8'}{/if}" method="post">
                   ...
                   <input name="qty" type="hidden" class="form-control atc_qty" value="1" onfocus="if(this.value == '1') this.value = '';" onblur="if(this.value == '') this.value = '1';"/>
                    <button class="add_to_cart btn btn-primary" {if $product.quantity > 0} onclick="mypresta_productListCart.add({literal}$(this){/literal});" {else} disabled {/if}>
