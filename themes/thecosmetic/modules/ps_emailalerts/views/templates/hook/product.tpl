...
<div class="js-mailalert" data-url="{url entity='module' name='ps_emailalerts' controller='actions' params=['process' => 'add']}">
	{if isset($email) AND $email}
        <span class="control-label">{l s='Mail to' d='Shop.Theme.Actions'}</span>
		<input type="email" placeholder="{l s='your@email.com' d='Shop.Theme.Actions'}"/>
	{/if}
  <input type="hidden" value="{$id_product|escape:'html':'UTF-8'}"/>
  <input type="hidden" value="{$id_product_attribute|escape:'html':'UTF-8'}"/>
	<a href="#" rel="nofollow" onclick="return addNotification();">{l s='Notify me when available' d='Shop.Theme.Actions'}</a>
